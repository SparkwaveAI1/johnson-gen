/**
 * Import Script: Arrowhattocks Extraction Batch 1
 *
 * This script imports extracted data from Va-1607-Arrowhattocks.pdf
 * into the Supabase database.
 *
 * Run with: node scripts/import-arrowhattocks-batch1.js
 */
...
const extractionPath = '../../sources/arrowhattocks-extraction-batch1.json'
...
async function importSources() {
  ...
  for (const source of extraction.sources_to_add) {
    const sourceData = {
      ...
      record_date: source.record_date || null,
...
async function createNewPeople() {
  ...
  for (const person of extraction.new_people_to_create) {
    ...
    const idMatch = person.suggested_id.match(/e(\d{4})/)
    if (idMatch) {
    ...
    if (error) {
      ...
      if (reciprocalType) {
        const reciprocalData = {
          ...
          relationship_status: rel.confidence,
...
async function runImport() {
  console.log('========================================')
  console.log('ARROWHATTOCKS EXTRACTION BATCH 1 IMPORT')
  console.log('========================================')
